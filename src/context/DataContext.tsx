...
interface DataContextValue {
  ...
  // hero
  addSlide: (
    slide: Omit<SlideItem, "id" | "order" | "image" | keyof CloudinaryMeta>,
    upload: CloudinaryUploadResult,
  ) => void;
  updateSlide: (
    id: string,
    patch: Partial<Omit<SlideItem, keyof CloudinaryMeta>>,
  ) => void;
  ...
  addImages: (
    ...
    common: Omit<
      GalleryImage,
      "id" | "createdAt" | "image" | keyof CloudinaryMeta
    >,
  ) => void;
  updateImage: (
    id: string,
    patch: Partial<Omit<GalleryImage, keyof CloudinaryMeta>>,
  ) => void;
  ...
  updateContact: (
    patch: Partial<
      Omit<
        ContactData,
        "contactBanner" | "contactBannerPublicId" | "contactBannerDeleteToken"
      >
    >,
  ) => void;
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addSlide: DataContextValue["addSlide"] = useCallback(
    (slide, upload) => {
      setData((prev) => ({
        ...prev,
        hero: {
  ...
  const updateSlide: DataContextValue["updateSlide"] = useCallback(
    (id, patch) => {
      setData((prev) => ({
        ...prev,
        hero: {
  ...
  const replaceSlideImage: DataContextValue["replaceSlideImage"] = useCallback(
    (id, upload) => {
      setData((prev) => ({
        ...prev,
        hero: {
          slides: prev.hero.slides.map((s) =>
            s.id === id ? { ...s, ...toImagePatch(upload) } : s,
          ),
        },
      }));
    },
  ...
  const deleteSlide: DataContextValue["deleteSlide"] = useCallback((id) => {
    setData((prev) => {
      ...
      if (target?.imageDeleteToken)
        void deleteFromCloudinary(target.imageDeleteToken);
      return {
        ...prev,
  ...
  const reorderSlides: DataContextValue["reorderSlides"] = useCallback(
    (orderedIds) => {
      setData((prev) => ({
        ...prev,
        hero: {
  ...
  const updateBiography: DataContextValue["updateBiography"] = useCallback(
    (patch) => {
      setData((prev) => ({
        ...prev,
        biography: { ...prev.biography, ...patch },
      }));
    },
    [],
  );

  const addAboutImage: DataContextValue["addAboutImage"] = useCallback(
    (alt, upload) => {
      setData((prev) => ({
        ...prev,
        biography: {
  ...
  const replaceAboutImage: DataContextValue["replaceAboutImage"] = useCallback(
    (id, upload) => {
      setData((prev) => ({
        ...prev,
        biography: {
  ...
  const deleteAboutImage: DataContextValue["deleteAboutImage"] = useCallback(
    (id) => {
      setData((prev) => {
        ...
        return {
          ...prev,
          biography: { ...prev.biography, aboutImages: remaining },
        };
      });
    },
    [],
  ...
  const setPrimaryAboutImage: DataContextValue["setPrimaryAboutImage"] =
    useCallback((id) => {
      setData((prev) => ({
        ...prev,
        biography: {
  ...
  const addImages: DataContextValue["addImages"] = useCallback(
    (uploads, common) => {
      setData((prev) => ({
        ...prev,
        gallery: [
  ...
  const updateImage: DataContextValue["updateImage"] = useCallback(
    (id, patch) => {
      setData((prev) => ({
        ...prev,
  ...
  const replaceImage: DataContextValue["replaceImage"] = useCallback(
    (id, upload) => {
      setData((prev) => ({
        ...prev,
        gallery: prev.gallery.map((g) =>
          g.id === id ? { ...g, ...toImagePatch(upload) } : g,
        ),
      }));
    },
    [],
  ...
  const bulkDeleteImages: DataContextValue["bulkDeleteImages"] = useCallback(
    (ids) => {
      ...
      setData((prev) => {
        ...
        return {
          ...prev,
          gallery: prev.gallery.filter((g) => !idSet.has(g.id)),
        };
      });
    },
    [],
  ...
  const updateContact: DataContextValue["updateContact"] = useCallback(
    (patch) => {
  ...
  const replaceContactBanner: DataContextValue["replaceContactBanner"] =
    useCallback((upload) => {
      setData((prev) => ({
        ...prev,
        contact: {
  ...
  const clearContactBanner: DataContextValue["clearContactBanner"] =
    useCallback(() => {
      setData((prev) => {
        if (prev.contact.contactBannerDeleteToken)
          void deleteFromCloudinary(prev.contact.contactBannerDeleteToken);
        return {
          ...prev,
          contact: {
            ...prev.contact,
            contactBanner: "",
            contactBannerPublicId: undefined,
            contactBannerDeleteToken: undefined,
          },
        };
      });
    }, []);
  ...
  const addMessage: DataContextValue["addMessage"] = useCallback((msg) => {
    setData((prev) => ({
      ...prev,
      messages: [
        { ...msg, id: uid(), createdAt: Date.now(), read: false },
        ...prev.messages,
      ],
    }));
  }, []);

  const markMessageRead: DataContextValue["markMessageRead"] = useCallback(
    (id, read) => {
      setData((prev) => ({
        ...prev,
  ...
  const updateSettings: DataContextValue["updateSettings"] = useCallback(
    (patch) => {
      setData((prev) => ({
        ...prev,
